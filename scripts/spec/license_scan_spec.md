...
❌ repo-a  [GPL-3.0 (my-lib), AGPL-3.0 (other-lib)]
     GPL-3.0: my-lib@1.0.0
     AGPL-3.0: other-lib@2.0.0
...
The failing check label in brackets shows each unique license found, with the package name in parentheses. The indented lines below show the full `package@version` for each violation.

Icons: `✅` pass, `❌` fail (active repos). Archived, unavailable, and excluded repos are listed with `–` (no scan performed).

Each active repo always prints a `packages / unique licenses` line before its result. With `--debug`, additional API call details are printed before the Results section.
