...
import { loadConfig, parseRelativeDate, processRepos, buildOspoArchiveIssueBody } from './archive_repos.js';
...
// ── buildOspoArchiveIssueBody ─────────────────────────────────────────────────

describe('buildOspoArchiveIssueBody', () => {
  it('generates correct table rows with repo and days until archival', () => {
    const body = buildOspoArchiveIssueBody([{ repo: 'my-repo', daysLeft: 32 }]);
    assert.ok(body.includes('| my-repo | 32 days |'));
  });

  it('generates rows for multiple repos', () => {
    const body = buildOspoArchiveIssueBody([
      { repo: 'repo-a', daysLeft: 10 },
      { repo: 'repo-b', daysLeft: 25 },
    ]);
    assert.ok(body.includes('| repo-a | 10 days |'));
    assert.ok(body.includes('| repo-b | 25 days |'));
  });

  it('includes the table header', () => {
    const body = buildOspoArchiveIssueBody([{ repo: 'r', daysLeft: 1 }]);
    assert.ok(body.includes('| Repository | Days until archival |'));
  });

  it('includes attribution line', () => {
    const body = buildOspoArchiveIssueBody([{ repo: 'r', daysLeft: 1 }]);
    assert.ok(body.includes('_This issue was automatically generated by the OSPO archive script._'));
  });
});
