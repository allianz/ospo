...
describe('buildOspoLicenseIssueBody', () => {
  it('generates correct table rows for one repo with one violation', () => {
    const body = buildOspoLicenseIssueBody([
      { repo: 'repo-a', violations: [{ name: 'some-lib', version: '1.2.3', spdxId: 'GPL-3.0' }] },
    ]);
    assert.ok(body.includes('| repo-a | some-lib | 1.2.3 | GPL-3.0 |'));
  });

  it('generates rows for multiple repos and violations', () => {
    const body = buildOspoLicenseIssueBody([
      {
        repo: 'repo-a',
        violations: [
          { name: 'lib-a', version: '1.0.0', spdxId: 'GPL-3.0' },
          { name: 'lib-b', version: '2.0.0', spdxId: 'AGPL-3.0' },
        ],
      },
      { repo: 'repo-b', violations: [{ name: 'bad-pkg', version: '0.1.0', spdxId: 'GPL-2.0' }] },
    ]);
    assert.ok(body.includes('| repo-a | lib-a | 1.0.0 | GPL-3.0 |'));
    assert.ok(body.includes('| repo-a | lib-b | 2.0.0 | AGPL-3.0 |'));
    assert.ok(body.includes('| repo-b | bad-pkg | 0.1.0 | GPL-2.0 |'));
  });

  it('includes the introductory sentence and table header', () => {
    const body = buildOspoLicenseIssueBody([
      { repo: 'r', violations: [{ name: 'x', version: '1', spdxId: 'GPL-3.0' }] },
    ]);
    assert.ok(body.includes('The following repositories contain dependencies'));
    assert.ok(body.includes('| Repository | Package | Version | License |'));
  });
